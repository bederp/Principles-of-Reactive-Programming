...
//   }
    ...
    test("Recovered given test case 1") {
    val requestStream = Observable.from(1 to 3) 
    def requestMethod(num: Int) = if (num != 2) Observable.just(num) else Observable.error(new Exception) 
    val actual = requestStream.concatRecovered(requestMethod).toBlocking.toList
    assert(actual.size == 3)
    assert(actual.toString == "List(Success(1), Failure(java.lang.Exception), Success(3))") 
   }

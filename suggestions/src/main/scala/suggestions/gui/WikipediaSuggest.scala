...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map { x => suggestionList.selection.items.toList(0) }.filter { x => println(x); x.isEmpty() }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.flatMap { x => 
      println(x)
      wikiPageResponseStream(x) }.recovered

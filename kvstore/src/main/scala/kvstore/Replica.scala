 ...
 */
  ...
  def persist(client: ActorRef, key: String, valueOption: Option[String], id: Long)(onTimeout: Option[ActorRef => Unit] = None): Unit = {
    valueOption match {
      case Some(value) =>
        kv += (key -> value)
      case None =>
  ...
  def persisted(key: String, id: Long, onPersisted: (ActorRef, Persist) => Unit): Unit = {
    waitingPersistence.get(id).map {
      case (sender, p, cancellable) =>
        cancellable.cancel()
        waitingPersistence -= id
        onPersisted(sender, p)
    }
  }
  def replicate(client: ActorRef, p: Persist): Unit = {
    secondaries.size > 0 match {
      case true =>
  ...
  def cancelInitialReplication(id: Long): Unit = {
    waitingInitialReplicates.get(id).map {
      case (_, waiting, _) =>
        waiting foreach {
          case (_, cancellable) =>
            cancellable.cancel()
